...
function query($query) {
  ...
  if (mysqli_num_rows($result)==0){
    ...
    <div class='mx-auto w-[800px] h-[400px] rounded-3xl from-cyan-200 via-white to-sky-300 bg-gradient-to-br shadow-cyan-500 shadow-lg p-20 text-center'>
      ...
      <h2 class='text-2xl font-serif leading-tight shadow-lg text-black'>No posts to display</h2>
...
$posts = query ("SELECT * FROM posts where user_id='$authUserId' order by id desc"); ?>
...
<div class="grid grid-cols-2 gap-4 items-center mt-32 mb-10 ml-[450px]">
  
<div class="w-[500px] rounded hover:shadow-lg shadow-md hover:shadow-cyan-800 items-center ml-32 bg-white  mb-8">
  <a href="post-show.php?id=<?=$post['id'];?>">
  <img class="w-full" src="http://localhost:3000/panel/posts/uploads/<?php echo $post['image_path']; ?>" alt="">
 </a> <div class="px-6 py-4">
    <div class="font-bold text-xl mb-2">
    <a class="font-mono text-gray-500 hover:underline" href="post-show.php?id=<?=$post['id'];?>"><?= $post["title"]; ?></a>
      </div>
